...
Player::Player() {
		...
		m_TileSize = 0;
		// анимация игрока
		auto& idleForward = m_AnimPlayer.CreateAnimation("idleForward", "graphics/player.png",  seconds(0.5), true);
		idleForward.AddFrames( Vector2i(0, 0),  Vector2i(135, 105), 4, 1);
		auto& dead = m_AnimPlayer.CreateAnimation("dead", "graphics/player.png",  seconds(0.5), false);
		dead.AddFrames( Vector2i(405, 0),  Vector2i(135, 105), 4, 1);
		m_AnimPlayer.SwitchAnimation("idleForward");
		m_AnimPlayer.Update( seconds(0));
		// устанавливаем координаты спрайта в центр 
		m_Sprite.setOrigin(m_Sprite.getGlobalBounds().width / 2, m_Sprite.getGlobalBounds().height / 2);		
}

// появление игрока в мире
void Player::spawn( IntRect planet,  Vector2f resolution, int tileSize) {
...
// область нахождения игрока
 FloatRect Player::getPosition() const {
	...
	auto myGlobalBounds =  FloatRect(m_Sprite.getGlobalBounds().left+20,
...
void Player::update( Time deltaTime,  Vector2i mousePosition)
{
	...
	// начало раздела если игрок полон сил
		...
		if (m_time_moving>  microseconds(5000)) {

			m_time_moving =  microseconds(0);
