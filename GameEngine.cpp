...
GameEngine::GameEngine() {
	...
	m_window.create( VideoMode(m_resolution.x, m_resolution.y), L"Проект С++",  Style::Fullscreen);
	// этап загрузки игры
    ...
    //загрузка массива музыки 
	 vector< string> str{ "sound/level1.wav","sound/plasma.wav","sound/mobv.wav","sound/per.wav",
	...
	// Загрузка рекорда 
	 ifstream inputFile("gamedata/scores.txt",  ios::binary |  ios::in);
	...
	while (m_window.pollEvent(event))
	{
		if (event.type ==  Event::KeyPressed) {
			...
			if (m_state == State::playing || m_state == State::transition) {
				...
				// Игровая пауза
				if (event.key.code ==  Keyboard::Pause && m_state != State::transition) {
				...
				// помощь в игре
				if (event.key.code ==  Keyboard::Tab && m_state != State::transition) {
		...
		{
			if (event.type ==  Event::MouseButtonPressed) {  // нажатие клавиш мыши
...
void GameEngine::update( Time const& deltaTime) {
	// Рандом
	 random_device rd; 
	 mt19937 gen(rd());              
	 uniform_int_distribution<> tis(1, 20);
	///------конец рандома -----------------------
	...
	if (m_state == State::playing || m_state == State::transition)
	{
		...
		m_player.update(deltaTime,  Mouse::getPosition());
		...
		// Область видимости игрока
		 Vector2f minview{ m_player.getSprite().getPosition().x - m_resolution.x / 2, m_player.getSprite().getPosition().y - m_resolution.y / 2 };
		 Vector2f maxview{ m_player.getSprite().getPosition().x + m_resolution.x / 2, m_player.getSprite().getPosition().y + m_resolution.y / 2 };
...
void GameEngine::newLevel() {
	...
	m_monster.clear();
	m_HubText.numMonsterAlive = createHorde(50 * m_HubText.level, m_monster,  Vector2i(0, m_HubText.level - 1), m_planet, m_HubText.complexity);
...
{
	 ofstream outputFile("gamedata/scores.txt",  ios::binary |  ios::out);
