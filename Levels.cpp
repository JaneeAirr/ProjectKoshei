...
Levels::Levels( RenderWindow& win,float scale_x, float scale_y):m_window(win)
{
 ...
 Sprite* Levels::getSprite(int index)
{
...
void Levels::splash_SCR_update( Time deltaTime)
{
	m_time_frame += deltaTime;

	if (m_sound[m_frame].getStatus() ==  SoundSource::Status::Stopped && m_play) {
		m_sound[m_frame].play(); m_play = false;
		}

	if (m_time_frame >  seconds(m_time[m_frame])) {
	
		m_time_frame =  seconds(0);

		if (m_frame < 6) {
			m_frame++; m_play = true;
			int step = 1920 * m_frame;
			m_splash_screen.setTextureRect( IntRect(step, 0, 1920, 1080));
		}
		else vstup = true;
	}
}
...
{
	 array< string, 7> str{ "graphics/level1.jpg", "graphics/level2.jpg", "graphics/level3.jpg",
	...
	m_splash_screen.setTextureRect( IntRect(0, 0, 1920, 1080));
	m_splash_screen.setScale(m_scale_x, m_scale_y);

	 array< string, 7> str1{ "sound/r1.wav", "sound/r2.wav", "sound/r3.wav",
